...
**Ask these questions together when the environment supports structured user input; otherwise ask them concisely in one message** so the user can answer everything at once:
...
2. **View each image** — Use Codex image viewing for local files, or browser/image inspection when appropriate
3. **Evaluate** — For each: what it shows, USABLE or NOT USABLE (with reason), what concept it represents, dominant colors
4. **Co-design the outline** — Curated images inform slide structure alongside text. This is NOT "plan slides then add images" — design around both from the start (e.g., 3 screenshots → 3 feature slides, 1 logo → title/closing slide)
5. **Confirm with the user** (header: "Outline"): "Does this slide outline and image selection look right?" Options: Looks good / Adjust images / Adjust outline
...
2. **Open** — Use the Codex in-app browser for local targets when available, or provide the local HTML path
